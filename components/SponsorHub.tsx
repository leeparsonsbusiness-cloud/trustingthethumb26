...
import MediaDeckModal from "./MediaDeckModal";
...
export default function SponsorHub() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!companyName || !email) {
      setErrorMsg("Please fill in your company name and contact email.");
      return;
    }
  ...
  const categories = [
    {
      icon: Shirt,
      title: "Outdoor & Layering Gear",
      examples: "Backpacks, boots, weather-proof jackets, socks",
    },
    {
      icon: BatteryCharging,
      title: "Portable Power & Solar",
      examples: "Power banks, solar chargers, rugged cables",
    },
    {
      icon: Apple,
      title: "Nutrition & Hydration",
      examples: "Energy bars, electrolyte mix, beef jerky, water filters",
    },
    {
      icon: Smartphone,
      title: "Travel Tech & Hardware",
      examples: "Action cameras, satellite communicators, audio gear",
    },
  ];

  return (
    <section id="sponsors" className="py-24 bg-asphalt-darker relative overflow-hidden border-t border-asphalt-border/40">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-desert/15 border border-amber-desert/40 text-amber-desert text-xs font-mono font-bold uppercase shadow-amber-glow">
            ...
            Open To Brand Partnerships & Gear Sponsors
          </div>
          ...
          <p className="text-base sm:text-lg text-parchment-muted leading-relaxed">
            Partner with Lee and Jake as they put products through 2,000 miles of extreme real-world highway testing across TikTok, Instagram Reels, and YouTube.
          </p>

          <div className="pt-2">
            <button
              onClick={() => setDeckModalOpen(true)}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-asphalt-card border border-asphalt-border text-parchment hover:text-amber-desert hover:border-amber-desert/40 text-xs font-mono font-bold transition-all shadow-inner"
            >
        ...
        {/* 3 Value Propositions for Marketing Managers */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="bg-asphalt-card/80 p-6 rounded-3xl border border-asphalt-border hover:border-amber-desert/40 transition-colors space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {categories.map((cat) => {
              ...
              return (
                <div
                  key={cat.title}
                  className="bg-asphalt-card/60 p-5 rounded-2xl border border-asphalt-border/60 flex items-start gap-4"
                >
        ...
        {/* Direct Sponsor Contact Form Card */}
        <div className="max-w-3xl mx-auto bg-asphalt-card rounded-3xl p-6 sm:p-10 border border-asphalt-border shadow-2xl space-y-6">
          <div className="text-center space-y-2 border-b border-asphalt-border/60 pb-6">
            ...
            <p className="text-xs text-parchment-muted font-mono">
              Direct inbox line to Lee & Jake&apos;s partnership management. Responded within 24h.
            </p>
          </div>
          ...
          {success && (
            <div className="p-4 rounded-2xl bg-sage/20 border border-sage/50 text-sage text-sm font-semibold flex items-center gap-3 animate-in fade-in">
              ...
              <span>Inquiry received! The brothers will review your pitch and email back shortly.</span>
            </div>
          )}
